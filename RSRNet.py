...
class RSRNetwork(object):
    def __init__(self,
        ...
        self.outputs = tf.concat([outputs_, signal_vec], 1)#Road Segment Representation和Normal Route Feature

        # self.logits2 = tf.layers.dense(self.outputs, num_units) #预测label
        self.logits = tf.layers.dense(self.outputs, 2)  # 预测label

        self.loss = tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels, logits=self.logits), name='loss') #计算预测损失
